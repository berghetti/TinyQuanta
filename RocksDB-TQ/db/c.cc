...
/* added by ZL */
void rocksdb_get_in_place(
    rocksdb_t* db,
    const rocksdb_readoptions_t* options,
    const char* key, size_t keylen, char* val,
    size_t* vallen,
    char** errptr) {
  std::string tmp;
  Status s = db->rep->Get(options->rep, Slice(key, keylen), &tmp);
  if (s.ok()) {
    *vallen = tmp.size();
    memcpy(val, tmp.data(), sizeof(char) * tmp.size());
  } else {
    *vallen = 0;
    if (!s.IsNotFound()) {
      SaveError(errptr, s);
...
void rocksdb_scan(
     ...
     const rocksdb_readoptions_t* options) {
    size_t klen;
    rocksdb_iterator_t *iter = rocksdb_create_iterator(db, options);
...
/* added by ZL */
rocksdb_slicetransform_t* rocksdb_slicetransform_create_capped_prefix(size_t prefixLen) {
  Wrapper* wrapper = new Wrapper;
  wrapper->rep_ = rocksdb::NewCappedPrefixTransform(prefixLen);
  wrapper->state_ = nullptr;
  wrapper->destructor_ = &Wrapper::DoNothing;
  return wrapper;
}
