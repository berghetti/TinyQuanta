...
static uint8_t cpus[NUM_WORKER_THREADS + 1] = {2,  4,  6,  8,  10, 12, 14, 16,
...
static void pin_to_cpu(int cpu_id) {
  ...
  // if(ret == 0)
  //    std::cout << "Successfuly pin the current thread to cpu " << cpu_id <<
  //    std::endl;
}
...
void coro(int coro_id, job_info_t *&jinfo, coro_t::push_type &yield) {
  ...
  // 	//rocksdb_get_in_place(db, readoptions, key, strlen(key), val, &vallen,
  // &err);
  //       	assert(!err);
...
void process_rx_mbuf(struct rte_mbuf *rx_mbuf, coro_info_t *idle_coro,
  ...
  uint16_t *jtype = rte_pktmbuf_mtod_offset(rx_mbuf, uint16_t *,
  RTE_ETHER_HDR_LEN + sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_udp_hdr) +
  sizeof(uint32_t) ); idle_coro->jinfo->jtype = static_cast<job_type>(*jtype);
  idle_coro->jinfo->input_data = rte_pktmbuf_mtod_offset(rx_mbuf, char *,
  RTE_ETHER_HDR_LEN + sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_udp_hdr) +
  ...
  //#ifdef SERVER_LAT
  // idle_coro->jinfo->job_start_time = static_cast< struct
  // rte_pktmbuf_pool_private_with_start_tsc*
  // >(rte_mbuf_to_priv(rx_mbuf))->start_tsc; idle_coro->jinfo->rocksdb_hdr =
  // rte_rocksdb_hdr; #else #ifdef QUEUE_SIZE rte_rocksdb_hdr->run_ns =
  // rte_cpu_to_be_32(queue_size); #else rte_rocksdb_hdr->run_ns = 0; #endif
  //#endif

  //*(uint32_t*)((char*)buf_ptr + sizeof(struct rte_udp_hdr)) = *seq_num;
  // idle_coro->jinfo->output_data = buf_ptr + sizeof(struct rte_udp_hdr) +
  // sizeof(uint32_t);
...
//    rocksdb_slicetransform_create_capped_prefix(8);
...
//
//    //rocksdb_block_based_options_set_block_cache(block_options,
//    rocksdb_cache_create_lru_shard(32 << 20, 8));
//
...
// flexibility in which end to use
...
#ifdef TIME_STAGE
  uint64_t start, stage1_end, stage2_end, stage3_end, stage4_end,
      yield_end_time, stage1_cycles = 0, stage2_cycles = 0, stage3_cycles = 0,
                      stage4_cycles = 0, num_samples = 0, total_work_time = 0;
#endif
  ...
  // uint64_t total_num_quanta = 0, total_execution_cycles = 0, finished_jobs =
  // 0, prev_finished_jobs = 0;

  // printf("Worker %d initialize all worker coroutines\n", tid);

  for (int coro_id = 0; coro_id < NUM_WORKER_COROS; coro_id++) {
#ifdef STACKS_FROM_HUGEPAGE
    worker_coros[coro_id] =
        coro_t::pull_type(SimpleStack(stack_pool),
                          boost::bind(coro, coro_id, &job_infos[coro_id], _1));
    stack_pool += STACK_SIZE;
#else
    worker_coros[coro_id] = coro_t::pull_type(
        SimpleStack(), boost::bind(coro, coro_id, &job_infos[coro_id], _1));
#endif
  ...
  for (;;) {
    ...
    if (!busy_coros.empty()) {
      ...
      num_assigned_quanta =
          (num_assigned_quanta + dispatch_index <= DISPATCH_RING_DEQUEUE_PERIOD)
              ? num_assigned_quanta
...
// prioritize new jobs
    ...
    /*if(unlikely(tid == 0 && (finished_jobs - prev_finished_jobs) > 100000)) {
            std::cout << "Execution time: " << (float) total_execution_cycles *
    1000 * 1000 /(finished_jobs * rte_get_timer_hz()) << " us" << std::endl;
            prev_finished_jobs = finished_jobs;
            std::cout << "Average number of quanta for each job: " << (float)
...
static int run_server() {
  ...
  for (int wid = 0; wid < NUM_WORKER_THREADS; wid++) {
    ...
    worker_info_vec[wid].rx_mbuf_dispatch_q =
        rte_ring_create(name, DISPATCH_RING_SIZE, rte_socket_id(),
  ...
  for (int wid = 0; wid < NUM_WORKER_THREADS; wid++) {
    ...
    worker_info_vec[wid].rx_mbuf_return_q = rte_ring_create(
        name, RETURN_RING_SIZE, rte_socket_id(), RING_F_SP_ENQ | RING_F_SC_DEQ);
  }
#else
...
#endif
    worker_args[wid].rx_mbuf_dispatch_q =
        worker_info_vec[wid].rx_mbuf_dispatch_q;
#ifndef NEW_DISPATCHER
  ...
  for (;;) {
    ...
    for (i = 0; i < nb_rx; i += max_dispatch_size) {
      ...
      if (unlikely(nb_return != dispatch_size)) {
        ...
        // dispatcher may have stale information about the number of jobs each
        // worker has, hence force a return pull
        rte_pktmbuf_free_bulk(&rx_bufs[i + nb_return],
                              dispatch_size - nb_return);
        total_running_jobs += nb_return;
        tmp_w->num_running_jobs += nb_return;
#ifndef RAND_DISP
        worker_queue.push(tmp_w);
#endif
        // std::cout << "Packet drop: total number of running jobs " <<
    ...
    for (i = 0; i < nb_rx; i++) {
      tmp_w = worker_queue.top();
      worker_queue.pop();
      if (unlikely(rte_ring_enqueue(tmp_w->rx_mbuf_dispatch_q, rx_bufs[i]) <
                   0)) {
        // drop this packet
        // dispatcher may have stale information about the number of jobs each
        // worker has, hence force a return pull
...
#ifdef NEW_DISPATCHER
  mp = rte_mempool_create_empty(name, n, elt_size, cache_size, private_size,
                                socket_id, RTE_MEMPOOL_F_SC_GET);
#else
  mp = rte_mempool_create_empty(name, n, elt_size, cache_size, private_size,
                                socket_id,
...
static int dpdk_init(int argc, char *argv[]) {
  ...
  rx_mbuf_pool = rte_pktmbuf_pool_create_spsc(
      "MBUF_RX_POOL", RX_MBUF_POOL_SIZE, RX_MBUF_CACHE_SIZE, 0,
  ...
  /* Creates a new mempool in memory to hold the mbufs. */
  tx_mbuf_pool = rte_pktmbuf_pool_create_w_customized_init(
      "MBUF_TX_POOL", TX_MBUF_POOL_SIZE, TX_MBUF_CACHE_SIZE, 0,
      RTE_MBUF_DEFAULT_BUF_SIZE, rte_socket_id());
